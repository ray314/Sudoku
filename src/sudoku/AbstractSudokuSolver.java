...
public abstract class AbstractSudokuSolver{
    ...
    protected int S; // size of the board
    protected int side; // how long the side is
    protected int[][] board;
    protected boolean boardAdded;
    public ArrayList<int[][]> filledBoards;
    
    // prints out all possible solutions. returns false if `sudoku` is invalid
    // to be implemented by concrete classes
    protected abstract void runSolver(int[][] sudoku);
    
    public AbstractSudokuSolver(int size, int side)
    {
    	this.S = size;
    	this.side = side;
    	filledBoards = new ArrayList<int[][]>();
    	board = new int[S][S];
    	boardAdded = false;
    }
    ...
    public void addBoard(int[][] result){ // add board to the list
        int N = result.length;
        for(int i = 0; i < N; i++){
            for(int j = 0; j < N; j++){
    ...
    protected static boolean validateSudoku(int[][] grid){
        if (grid.length != 9 && grid.length != 12)
            return false; // only 9 or 12 for now
